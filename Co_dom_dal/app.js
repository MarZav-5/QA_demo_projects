...
function recipeToHtml(r) {
    return `
    <h2>${escapeHtml(r.title)} <small>• 2 porcie • ~${Number(r.timeMinutes) || 15} min</small></h2>
    <h3>Ingrediencie</h3>
    <ul class="list">
...
function showRecipe(html){
    ...
    setTimeout(() => {
      ...
      // zobraz recept a resetuj scroll NA VRCH
      viewRecipe.classList.remove('hidden');
      viewRecipe.classList.add('wiping-in');
  
      // === DÔLEŽITÉ: reset scrollu na viac úrovniach ===
    ...
    setTimeout(() => {
      ...
      // poistka: po dokončení animácie ešte raz na vrch
      viewRecipe.scrollTop = 0;
      document.querySelector('.sheet')?.scrollTo?.(0,0);
    }, 460);
  }
  ...
  function showForm(){
    // pri návrate na formulár takisto na vrch
...
$('gen').onclick = async () => {
    ...
    // simulácia latencie, nech vidíš loader a wipe
    setTimeout(() => {
        const fromDB = findRecipeByIngredients(ings);
        const data = fromDB || mockRecipe(ings);
        showRecipe(recipeToHtml(data));
        statusEl.textContent = fromDB ? '' : '⚠️ Fallback z lokálnych dát.';
        setLoading(false);
    }, 600);
};

// --- Návrat späť ------------------------------------------------------------
$('edit').onclick = () => { setLoading(false); showForm(); };
